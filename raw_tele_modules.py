...
def weekly_times(name,tstart,tend):
    ...
    # next- fix all the bad names that are not like telemetry status names
    #print('replacing all the bad names not consistent with telemetry status names')
    df=df.replace(['llenDiane','Eliz&Katherine','Eliz and Katherine','Mystic Steve'],['Ellen_Diane','Elizabeth_Katherine','Elizabeth_Katherine','Mystic'])   
    df=df[df['name'].replace(' ','_')==name]
    for j in df.index:
        if not tstart<=df['time'][j]<=tend:
            df=df.drop(j)
    #print('finished finding report cases')        
    if len(df)==0:
        return {'yes': 0, 'no': 0}
    dorklist=[]
    for i in df.index:
    ...
    print('finished finding '+name+' reports away from the dock')        
    if len(dorklist)==0:  
        return {'yes': 0, 'no': 0}
...
#        sys.exit()
    ...
    raw_dict={}    #the dictinary about raw data, use to write the data about 'time','filename','mean_temp','mean_depth'
    tele_dict={}  #the dictionary about telementry data,use to write the data about'time','mean_temp','mean_depth'
    emolt_raw_dict={}#the dictinary about raw data, use to write the data about 'vessel','datet','lat','lon','depth','depth_range','hours','mean_temp','std_temp'
    ...
    for file in file_lists: # loop raw files   
         ...
         try:
            ...
            continue                                        
    print('finished looping through raw files')                            
    for i in record_file_df.index:
        ...
        if record_file_df['file_number'].isnull()[i]:
            record_file_df['file_number'][i]=0
    print('finished counting for statistics')
    for i in telemetrystatus_df['Boat']:#loop every boat,
        raw_dict[i]=raw_dict[i].sort_values(by=['time'])
        raw_dict[i].index=range(len(raw_dict[i]))
    ...
    print('finished calculating number of boats')
...
def read_telemetry(path):
    ...
    Modified by JiM 24 Sep 2020 to deal better with empty telemetry file"""
    #while True:
    ...
    #    break
    while len(tele_df)==0:        
        print('reading_telemetry did not have all the data so sleeping and waiting 10 minutes to redownload data')
        time.sleep(600)
        tele_df=pd.read_csv(path,sep='\s+',names=['vessel_n','esn','month','day','Hours','minates','fracyrday',\
                            'lon','lat','dum1','dum2','depth','rangedepth','timerange','temp','stdtemp','year'])
    return tele_df
