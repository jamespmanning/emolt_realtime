...
############  HARDCODES #####
ts=datetime(2022,3,5,0,0,0)
te=datetime(2022,6,16,0,0,0)
vessel='Mister_G'#'Rachel_Leah'#
outpath='data/'+vessel+'/'
case='weekly' # 'tidal', 'weekly','daily','single'
trip='20220616' # yearmmdd for case of 'tidal' or last day in case of 'weekly'
if case=='daily':
    files=['data/St_Croix/zt_0147_20220517_152127_St_Croix.csv',
           'data/St_Croix/zt_0147_20220518_165915_St_Croix.csv','data/St_Croix/zt_0147_20220518_184647_St_Croix.csv','data/St_Croix/zt_0147_20220519_150816_St_Croix.csv','data/St_Croix/zt_0147_20220520_200518_St_Croix.csv',
           'data/St_Croix/zt_0147_20220521_173841_St_Croix.csv','data/St_Croix/zt_0147_20220522_155705_St_Croix.csv','data/St_Croix/zt_0147_20220523_173249_St_Croix.csv','data/St_Croix/zt_0147_20220527_172955_St_Croix.csv',
           'data/St_Croix/zt_0147_20220529_154622_St_Croix.csv','data/St_Croix/zt_0147_20220529_191318_St_Croix.csv','data/St_Croix/zt_0147_20220531_154641_St_Croix.csv']

elif case=='weekly':
    files=['data/Mister_G/zt_0057_20220305_231238_Mister_G.csv','data/Mister_G/zt_0057_20220310_230804_Mister_G.csv',
       'data/Mister_G/zt_0057_20220316_181604_Mister_G.csv','data/Mister_G/zt_0057_20220318_202830_Mister_G.csv',
       'data/Mister_G/zt_0057_20220323_162238_Mister_G.csv','data/Mister_G/zt_0057_20220403_191815_Mister_G.csv',
       'data/Mister_G/zt_0057_20220415_195400_Mister_G.csv',
       'data/Mister_G/zt_0057_20220418_163322_Mister_G.csv','data/Mister_G/zt_0057_20220421_182003_Mister_G.csv',
       'data/Mister_G/zt_0057_20220504_132445_Mister_G.csv','data/Mister_G/zt_0057_20220616_191117_Mister_G.csv']
    
elif case=='tidal': #tidal set
    files=glob.glob(outpath+'*'+trip+'*'+vessel+'*.csv')#
    files= [s.replace('\\','/') for s in files] 
else:    
    files=['data/Rachel_Leah/zt_0133_20220611_135312_Rachel_Leah.csv']

colors=['green','red','blue','yellow','black','cyan','magenta','pink','grey','orange','brown','lime','plum']
...
# JiM modification:
for k in range(len(files)): # loop through multiple files
    data = pd.read_csv(files[k], header=8)
    #data['DATETIME'] = pd.to_datetime(data.Date + ' ' + data.Time, format='%Y-%m-%d %H:%M:%S')
    #data['DATETIME'] = pd.to_datetime(data['Datet(GMT)'], format='%Y-%m-%d %H:%M:%S')
    try:
        data['DATETIME'] = pd.to_datetime(data['datet(GMT)'], format='%Y-%m-%d %H:%M:%S')
    except:
        data['DATETIME'] = pd.to_datetime(data['datet(GMT)'], format='%m/%d/%Y %H:%M')#OE-M software as of June 2022
    data.rename(columns={'Temperature (C)': 'TEMPERATURE', 'Depth (m)': 'PRESSURE'}, inplace=True)
    data['TEMPERATURE'] = pd.to_numeric(data['TEMPERATURE'])
    data['PRESSURE'] = pd.to_numeric(data['PRESSURE'])
    data['TEMPERATURE'] = data.apply(lambda x: round(x['TEMPERATURE'], 6), axis=1)
    ...
    dfrbr=dfrbr[0::10]# subsample
    dfrbr['DATETIME'] = pd.to_datetime(dfrbr.Time, format='%m/%d/%Y %H:%M:%S')+timedelta(hours=5)#-timedelta(seconds=100)
    ...
    #dfrbr=dfrbr[dfrbr['DATETIME']>datetime(2022,1,24,16,20,3)] #38_1
    #dfrbr=dfrbr[dfrbr['DATETIME']<datetime(2022,1,24,16,21,30)]
    dfrbr=dfrbr[dfrbr['DATETIME']>ts]
    dfrbr=dfrbr[dfrbr['DATETIME']<te]
    ...
    #plt.savefig(files[k][14:-4]+'_time_series_'+case+'.png')
    #plt.close()
    ...
    #             zorder=1)
    #plt.plot(df_down['TEMPERATURE'], df_down['PRESSURE'], colors[k], label='Moana down '+files[k][25:27]+'/'+files[k][27:29]+'/2022', alpha=0.5, linewidth=10,
    #             zorder=1
    # weekly set
    #plt.plot(df_down['TEMPERATURE'], df_down['PRESSURE'], colors[k], label='Moana down '+filename[3:8]+'-20'+filename[0:2], alpha=0.5, linewidth=10,
    #         zorder=1)
    plt.plot(df_down['TEMPERATURE'], df_down['PRESSURE'], colors[k], label=filename[3:8]+'-20'+filename[0:2]+' at '+filename[9:14]+'Z',
             alpha=0.5, linewidth=10,zorder=1)
    if k==0:
        lab='temperature'
    else:
        lab='_nolegend_'
    #tem = plt.scatter(df['TEMPERATURE'], df['PRESSURE'], c=df['TEMPERATURE'],
    #                       cmap='coolwarm', label=lab, linewidth=5, zorder=3)
    ...
    #plt.title("F/V Rachel Leah in ~1000 feet off eastern Georges Bank "+datetime.utcfromtimestamp(df['DATETIME'].values[0].tolist()/1e9).strftime("%m/%d/%Y "), fontsize=16)
    ax.set_title("Subtle "+case+" changes in water column thermal structure as observed by F/V "+files[k][-12:-4],fontsize=16)#+datetime.utcfromtimestamp(df['DATETIME'].values[-1].tolist()/1e9).strftime("%m/%d/%Y, %H:%M:%S")+' UTC', fontsize=20)
...
#plt.savefig(files[k][-14:-4] + '_up_down.png')
plt.savefig('plots/'+case+'_'+vessel+'_'+trip+'_up_down.png')
