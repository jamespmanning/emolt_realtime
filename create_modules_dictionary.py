...
output file.p,in there have subdictionary to save every modules data in file.p
@author: leizhao

Modified by JiM in Jan 2020 w/more comments
"""
import pandas as pd
        ...
        for i in emolt_df.index:  #valuable_tele_df is the valuable telemetry data during start time and end time 
            if int(emolt_df['vessel_n'][i].split('_')[1])==telemetrystatus_df['Vessel#'][j]:
                ...
                try:
                    ...
                    ngdc_depth=np.nan   
                try:
                    climtemp=mm.getclim(lat1=latpt,lon1=lonpt,dtime=ptime)#store the data of climate history and ngdc
                except KeyboardInterrupt:
                    sys.exit()
                except:
                    climtemp=np.nan
...
def read_emolt_all(path='https://apps-nefsc.fisheries.noaa.gov/drifter/emolt.dat',endtime=datetime.now()):
    ...
    while True:
        ...
        if int(emolt_df['year'][len(emolt_df)-2])==endtime.year:
            ...
            print('check the web:https://apps-nefsc.fisheries.noaa.gov/drifter/emolt.dat.')
            time.sleep(600)
    return emolt_df

#def read_emolt(start,end,path='https://www.nefsc.noaa.gov/drifter/emolt.dat'):
def read_emolt(start,end,path='https://apps-nefsc.fisheries.noaa.gov/drifter/emolt.dat'):
...
def update_dictionary(telemetry_status,start_time,end_time,dictionarypath):
    '''use to update the dictonary that use to store the data from models
    ...
    try: #read dictionary
        ...
        dictionary={}  
    #update dictionary 
    obsdpogmf=classify_by_boat(telemetry_status,start_time,end_time,dictionary)  #running function to store new data from models
...
def main():
    ...
    update_dictionary(telemetry_status,start_time,end_time,dictionarypath)   #that is a function use to update the dictionary
